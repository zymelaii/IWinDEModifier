...
//  values will also be supported unless IMGUI_DISABLE_OBSOLETE_KEYIO is set] [X] Platform: Gamepad
//  support. Enabled with 'io.ConfigFlags |= ImGuiConfigFlags_NavEnableGamepad'. [X] Platform: Mouse
//  cursor shape and visibility. Disable with 'io.ConfigFlags |=
...
static void ImGui_ImplWin32_AddKeyEvent(ImGuiKey key, bool down, int native_keycode,
										int native_scancode = -1) {
	ImGuiIO& io = ImGui::GetIO();
	io.AddKeyEvent(key, down);
	io.SetKeyEventNativeData(
		key, native_keycode, native_scancode);	 // To support legacy indexing (<1.87 user code)
...
static void ImGui_ImplWin32_UpdateGamepads() {
	...
	ImGuiIO&			  io = ImGui::GetIO();
	ImGui_ImplWin32_Data* bd = ImGui_ImplWin32_GetBackendData();
	// if ((io.ConfigFlags & ImGuiConfigFlags_NavEnableGamepad) == 0) // FIXME: Technically feeding
	// gamepad shouldn't depend on this now that they are regular inputs.
	//     return;
	...
	if (!bd->HasGamepad || bd->XInputGetState == NULL ||
...
// There is no distinct VK_xxx for keypad enter, instead it is VK_RETURN + KF_EXTENDED, we assign it
// an arbitrary value to make code more readable (VK_ codes go up to 255)
...
// inputs to Dear ImGui, and hide them from your application based on those two flags. PS: In this
// Win32 handler, we use the capture API (GetCapture/SetCapture/ReleaseCapture) to be able to read
// mouse coordinates when dragging mouse outside of our window bounds. PS: We treat DBLCLK messages
// as regular mouse down messages, so this code will work on windows classes that have the
// CS_DBLCLKS flag set. Our own example app code doesn't set this flag.
#if 0
	...
	switch (msg) {
		...
		case WM_SYSKEYUP: {
			...
			if (wParam < 256) {
				...
				// keypad enter, see IM_VK_KEYPAD_ENTER definition for details, it is mapped to
				...
				if (vk == VK_SHIFT) {
					...
					if (IsVkDown(VK_LSHIFT) == is_key_down) {
						ImGui_ImplWin32_AddKeyEvent(
							ImGuiKey_LeftShift, is_key_down, VK_LSHIFT, scancode);
					}
					if (IsVkDown(VK_RSHIFT) == is_key_down) {
						ImGui_ImplWin32_AddKeyEvent(
							ImGuiKey_RightShift, is_key_down, VK_RSHIFT, scancode);
					}
				} else if (vk == VK_CONTROL) {
					if (IsVkDown(VK_LCONTROL) == is_key_down) {
						ImGui_ImplWin32_AddKeyEvent(
							ImGuiKey_LeftCtrl, is_key_down, VK_LCONTROL, scancode);
					}
					if (IsVkDown(VK_RCONTROL) == is_key_down) {
						ImGui_ImplWin32_AddKeyEvent(
							ImGuiKey_RightCtrl, is_key_down, VK_RCONTROL, scancode);
					}
				} else if (vk == VK_MENU) {
					if (IsVkDown(VK_LMENU) == is_key_down) {
						ImGui_ImplWin32_AddKeyEvent(
							ImGuiKey_LeftAlt, is_key_down, VK_LMENU, scancode);
					}
					if (IsVkDown(VK_RMENU) == is_key_down) {
						ImGui_ImplWin32_AddKeyEvent(
							ImGuiKey_RightAlt, is_key_down, VK_RMENU, scancode);
					}
				}
			}
			return 0;
		...
		case WM_IME_CHAR: {
			...
			DWORD wChar = wParam;
			if (wChar <= 127 &&
				false /* prevent conflictions between input engine and WM_CHAR message*/) {
				io.AddInputCharacter(wChar);
			} else {
...
#endif
		case WM_SETCURSOR:
			// This is required to restore cursor when transitioning from e.g resize borders to
			// client area.
...
static BOOL _IsWindowsVersionOrGreater(WORD major, WORD minor, WORD) {
	...
	return (RtlVerifyVersionInfoFn(
				&versionInfo, VER_MAJORVERSION | VER_MINORVERSION, conditionMask) == 0)
			   ? TRUE
			   : FALSE;
}
...
#if defined(_MSC_VER)
#	pragma comment( \
		lib, "dwmapi")	 // Link with dwmapi.lib. MinGW will require linking with '-ldwmapi'
#endif
