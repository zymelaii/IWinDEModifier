...
void PrivateOnlyHub::push(bool is_self, const std::string& msgid, const std::string& text) {
	json item{
		{"type", "private"},
		{"id", msgid},
...
nlohmann::json PrivateOnlyHub::parse(uint64_t id) {
	return *reinterpret_cast<json*>(request(id));
}

float PrivateOnlyHub::getScrollState() const {
	return scroll_state_;
}

void PrivateOnlyHub::scrollTo(float cursor) {
...
void PrivateOnlyHub::resize(ImVec2 size) {
...
void PrivateOnlyHub::renderScrollBar(const ImRect& bb) {
...
void PrivateOnlyHub::resolvePanel(const ImRect& bb) {
	...
	auto id		= window->GetID("##ChatHub.PrivateOnlyHub::Panel");
	bool hovered, held;
...
void PrivateOnlyHub::resolveInput(const ImRect& bb) {
	ImRect	Padding{16.00, 4.00, 16.00, 4.00};			   //!< 聊天面板内边距
	float	VertSpacing{6.00};							   //!< 消息项垂直间距
	float	InputHeight{48.00};							   //!< 输入栏高度
	ImRect	InputPadding{16.00, 4.00, 16.00, 4.00};		   //!< 输入栏内边距
	...
	const char*		label		  = "##ChatHub.PrivateOnlyHub::Input";
	...
	if (should_post) {
		char sid[32] = {0};
		sprintf(sid, "PrivateOnlyHub#%llu", nextid++);
		push(!!(rand() % 2), sid, typebuf);
	}
}

void PrivateOnlyHub::render(ImVec2 pos, ImVec2 size) {
	...
	auto   id = window->GetID("##ChatHub.PrivateOnlyHub");
	ImRect bb{{0, 0}, size};
...
bool PrivateOnlyHub::bindFactory(MsgItemFactory* factory) const {
	if (!factory) return false;
	factory_.reset(factory);
	return true;
}

bool PrivateOnlyHub::shouldRender() const {
	return should_render_;
}

ImVec2 PrivateOnlyHub::getMinSizeConstraint() const {
	ImRect padding{16.00, 4.00, 16.00, 4.00};		  //!< 聊天面板内边距
...
uint64_t PrivateOnlyHub::queryNext(uint64_t id) {
...
uint64_t PrivateOnlyHub::queryPrevious(uint64_t id) {
	if (id == 0) return items_.size() == 0 ? 0 : items_[0].first;
	uint64_t prev_id = 0;
	for (const auto& [this_id, item] : items_) {
...
void* PrivateOnlyHub::request(uint64_t id) {
...
bool PrivateOnlyHub::jumpToItem(uint64_t id) {
	return false;
}