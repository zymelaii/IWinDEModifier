...
public:
	...
	void resize(ImVec2 size) {
		...
		if (abs(size.x - size_.x) > 1e-3) scroll_range_.y = scroll_range_.x;
		size_ = size;
	}
	...
	void renderScrollBar(const ImRect& bb) {
		...
		ImRect bar_bb;
		bar_bb.Min.x = bb.Max.x - spacing - bar_width;
		bar_bb.Min.y = bb.Min.y + spacing + (total_height - bar_height) * scale;
		bar_bb.Max.x = bb.Max.x - spacing;
	...
	void resolvePanel(const ImRect& bb) {
		...
		auto id		= window->GetID("#PrivateOnlyHub.Panel");
		bool hovered, held;
		...
		if (ImGui::GetFocusID() == id || panel_bb.Contains(io.MousePos)) {
			scroolTo(getScrollState() - io.MouseWheel * 60.00);
		}
	}
	...
	void resolveInput(const ImRect& bb) {
		...
		const char*		label		  = "ChatHub Input";
		...
		input_size.x -= label_size.x + style.ItemInnerSpacing.x;
	...
	void render(ImVec2 pos, ImVec2 size) {
		...
		auto   id = window->GetID("#PrivateMsgFactory");
		ImRect bb{{0, 0}, size};
